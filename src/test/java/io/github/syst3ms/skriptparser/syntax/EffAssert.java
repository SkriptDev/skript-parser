...
public class EffAssert extends Effect {
    ...
    private String errorString(TriggerContext ctx) {
        if (this.message == null) {
            if (pattern == 0) {
                return "Assertion failed ('" + condition.toString(ctx, logger.isDebug()) + "', " + logger.getScript().scriptName() + ")";
            } else {
                if (shouldThrow) {
                    assert compiled;
                    return "Expected a non-parsable expression (" + matched + ", " + logger.getScript().scriptName() + ")";
                } else {
                    assert !compiled;
                    return "Expected a parsable expression (" + matched + ", " + logger.getScript().scriptName() + ")";
                }
            }
        } else {
            return message.getSingle(ctx).map(String::new).orElse("No message provided") + " (" + logger.getScript().scriptName() + ")";
        }
    }
}
